  ...
  async updateById(id, newData) {
    ...
    if(this._validateKeysExist(newData)){
      ...
      try {
        ...
        const data = await this.getData();
        const parsedData = JSON.parse(data);
        const objectIdToBeUpdated = parsedData.find((producto) => producto.id === id);
      ...
      console.log("this is the problem 3")
      return false;
    }
  }
  ...
  async addToArrayById(id, objectToAdd) {
    ...
    try {
      ...
      if (objectIdToBeUpdated) {
        ...
        const newArray = [...valorActual['products'], objectToAdd['products']];
        parsedData[index]['products'] = newArray;
        await fs.promises.writeFile(this._filename, JSON.stringify(parsedData));
        return true;
      } else {
          console.log(`ID ${id} does not exist in the file`);
